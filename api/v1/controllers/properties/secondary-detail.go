package properties

import (
	"encoding/json"
	"fmt"
	"net/http"
	"strconv"

	"kaya-backend/models"

	logger "kaya-backend/library/logger/v2"

	"github.com/gin-gonic/gin"
)

func (c propertiesController) DetailSecondary(ctx *gin.Context) {
	fmt.Println(">>> Detail - Controller <<<")

	log := logger.InitLogs(ctx.Request)

	res := models.Response{}

	slug := ctx.Params.ByName("slug")

	reqBytes, _ := json.Marshal(slug)
	log.Info("API - Property (Request)", log.AddField("RequestBody", string(reqBytes)))

	c.CustomerPropertySecondaryService.DetailSecondary(slug, &res)

	resBytes, _ := json.Marshal(res)
	log.Info("API - Property (Response)", log.AddField("ResponseBody", string(resBytes)))

	ctx.JSON(http.StatusOK, res)
}

func (c propertiesController) DetailSecondaryByID(ctx *gin.Context) {
	fmt.Println(">>> Detail - Controller <<<")

	log := logger.InitLogs(ctx.Request)

	res := models.Response{}

	reqID := ctx.Params.ByName("id")
	ID, _ := strconv.Atoi(reqID)

	reqBytes, _ := json.Marshal(ID)
	log.Info("API - Property (Request)", log.AddField("RequestBody", string(reqBytes)))

	c.CustomerPropertySecondaryService.DetailSecondaryByID(ID, &res)

	resBytes, _ := json.Marshal(res)
	log.Info("API - Property (Response)", log.AddField("ResponseBody", string(resBytes)))

	ctx.JSON(http.StatusOK, res)
}
