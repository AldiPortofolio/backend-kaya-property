...
type KayalogInterface interface {
...
func InitLogs(req *http.Request) KayalogInterface {
	uid := fmt.Sprintf("%v", uuid.New())
	span := strings.ReplaceAll(uid, "-", "")

	return &KayaLog{SpanID: span}
}

func (log *KayaLog) GetSpanID() string {
	return log.SpanID
}

func (log *KayaLog) AddField(fieldname string, value interface{}) LogField {
	return LogField{Key: fieldname, Value: value}
}

func (log *KayaLog) Info(message string, fields ...LogField) {
...
func (log *KayaLog) Warn(message string, fields ...LogField) {
	timeNow := time.Now().Format("2006-01-02 15:04:05")
	runtime := getCaller()
	msg := make(chan []byte)
	defer close(msg)
...
func (log *KayaLog) Error(message string, fields ...LogField) {
	timeNow := time.Now().Format("2006-01-02 15:04:05")
	runtime := getCaller()
	msg := make(chan []byte)
	defer close(msg)
...
func (log *KayaLog) Critical(message string, fields ...LogField) {
	timeNow := time.Now().Format("2006-01-02 15:04:05")
	runtime := getCaller()
	msg := make(chan []byte)
	defer close(msg)
...
func (l *KayaLog) processMessage(level string, message string, caller string, ts string, fields []LogField, msg chan []byte) {
	logObj := LogObj{
		Data: LogData{
			PublishTime: ts,
			LogLevel:    level,
...
func (l *KayaLog) publishToFile(obj LogObj) {
	//Create, Check path
	dir := "./logFiles"
...
func (log *KayaLog) sendAndPrintMessage(msg []byte) {
	if len(msg) < 1 {
...
func (l *KayaLog) sendToAPI(logObj interface{}) {
	requestAgent := gorequest.New()
	timeout, _ := time.ParseDuration(SEND_TIMEOUT)
	url := APIURL

	//Send request
	requestAgent = requestAgent.Post(url)
	_, body, errs := requestAgent.Send(logObj).Timeout(timeout).End()
	if errs != nil {
		log.Printf("Send Error, Body:%q Err:%+v\n", body, errs)
	}
}
